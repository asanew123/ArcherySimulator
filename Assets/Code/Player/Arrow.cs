...
{
    public float damage = 80;   // 기본 데미지
    float Head_M = 2;   // 헤드샷 배율
    float AP_P = 25;    // 방어 관통 확률(최대치);
    ...
    void Start()
    {
        ...
        speed = 5 * (0.1f * Play.Charge_Save);
        RotationRate = 25 - (Play.Charge_Save/5);

        damage = (8*(1+((float)Set_M.Arrow_Level/10))) * (0.1f * Play.Charge_Save);
        AP_P = (2.5f*(1+((float)Set_M.Arrow_Level/10))) * (0.1f * Play.Charge_Save);
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(other.tag == "Arrow" && Game_Mode == 1)
        {
            damage = 0;
            Play.damage = 0;
            Maximum_Rotate = true;
    ...
    void Wind_Effect()
    {
        if(speed > 0)
        {
            ...
            if(y_wind > 0)  // 위쪽 바람
            {
                transform.Translate(Vector3.forward * Time.deltaTime * y_wind * 2.5f);
            }
            if(y_wind < 0)  // 아래쪽 바람
            {
