...
{
    public float damage = 120;   // 기본 데미지
    float Head_M = 2.5f;   // 헤드샷 배율
    float AP_P = 10;    // 방어 관통 확률(최대치);  // 사냥용 화살은 방어관통에 성공해도 데미지가 절반만 들어간다
    ...
    public bool Robin_Arrow_Cheak = false;
    public float Robin_Arrow_Score = 0;
    ...
    void Start()
    {
        ...
        speed = 6 * (0.1f * Play.Charge_Save);
        RotationRate = 25 - (Play.Charge_Save/5);

        damage = (12 * (1+((float)Set_M.Arrow3_Level/10))) * (0.1f * Play.Charge_Save);
        AP_P = (1+(3 * ((float)Set_M.Arrow3_Level/10))) * (0.1f * Play.Charge_Save);
    ...
    void Update()
    {
        ...
        if(G_M.Arr3_Hiden_Skill == true && Player_Arrow == false)
        {
            if(speed > 0)
            {
                Play.Time_Stop = true;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(other.tag == "Slow Zone" || other.tag == "Enemy")
        {
            speed /= 3f;
            RotationRate /= 3f;
        ...
        if(other.tag == "Arrow3" && Game_Mode == 1)
        {
            damage = 0;
            Play.damage = 0;
            Maximum_Rotate = true;
    ...
    void Shoot_Wait_Time2()
    {
        if(Time_Wait == true)
        {
            ...
            if(Shoot_Wait_Time > Shoot_Next_Time)
            {
                ...
                Time_Wait = false;
                Play.Shoot_Mod = false;
                if(Play.Time_Stop == true && Play.Option_Use == false)
                {
                    Play.Time_Stop = false;
                }
    ...
    void Wind_Effect()
    {
        if(speed > 0)
        {
            ...
            if(y_wind > 0)  // 위쪽 바람
            {
                transform.Translate(Vector3.up * Time.deltaTime * y_wind);
            }
            if(y_wind < 0)  // 아래쪽 바람
            {
                transform.Translate(Vector3.up * Time.deltaTime * y_wind);
