...
public class Eagle_Fantasy_Head : MonoBehaviour
{
    public GameObject E;
    ...
    void Start()
    {
        ...
        Eag = E.transform.GetComponent<Eagle_Fantasy>();
    ...
    private void OnTriggerEnter(Collider other)
    {
        if(other.tag == "Arrow" && Play.damage > 0)
        {
            ...
            Eag.HP -= (Play.damage * Play.Head_M);
            Eag.Damage_Cheak = true;
            Shot_M.All_False();
            Shot_M.Shot_Text_Wait_Time = Shot_M.Wait_Time_Temp;
            Shot_M.HS = 1;
            Shot_M.Col = true;
            Play.damage = 0;
            Debug.Log("독수리(판타지) 머리 피격");
            if((Eag.HP -= (Play.damage * Play.Head_M)) <= 0)
            {
