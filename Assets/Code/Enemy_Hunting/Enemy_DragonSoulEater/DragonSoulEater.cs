...
public class DragonSoulEater : MonoBehaviour
{
    ...
    public Animator DragonSoulEater_anim;
    ...
    public float HP = 130;              // 기본 체력   Easy : 130       Normal : 180        Hard : 230
    public float Max_HP;
    public int Damage = 15;                  // 기본 데미지      Easy : 15    Normal : 25     Hard : 35;     
    ...
    void Update()
    {
        ...
        {
            DragonSoulEater_anim.SetBool("Time_Stop", false);
            Enemy_Ctrl();
        }
        else if(Play.Time_Stop == true)
        {
            DragonSoulEater_anim.SetBool("Time_Stop", true);
        }
    }
    ...
    void Enemy_Ctrl()
    {
        ...
        if(Death == false)
        {
            if(Attack_Use == true)
            {
                DragonSoulEater_anim.SetBool("Attack", true);
            ...
            else if(Attack_Use == false)
            {
                if(Damage_Cheak == true)
                {
                    ...
                    DragonSoulEater_anim.SetBool("Fast_Fly", true);
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            DragonSoulEater_anim.SetBool("Walk", true);
        }
    ...
    void Die()
    {
        if(HP <= 0)
        {
            HP = 0;
            Death = true;
            DragonSoulEater_anim.SetBool("Die", true);
            nav.enabled = false;
        }
        ...
        {
            death_time = 0;
            DragonSoulEater_anim.SetBool("Die_Cheak", true);
            rb.isKinematic = true;
            Destroy(this.gameObject, 2f);
            G_M.Kill_Count += 1;
            G_M.Kill_Cheak = true;
    ...
    void Enemy_UI()
    {
        HP_Bar.fillAmount = HP / (80 + (50 * Game_Level)); 
