...
public class Fox : MonoBehaviour
{
    ...
    public float HP = 60;              // 기본 체력   Easy : 20       Normal : 40        Hard : 60
    public float Max_HP;
    public int Damage = 15;                  // 기본 데미지      Easy : 4    Normal : 6     Hard : 8;     
    ...
    void Update()
    {
        ...
        {
            Fox_anim.SetBool("Time_Stop", false);
            Enemy_Ctrl();
        }
        else if(Play.Time_Stop == true)
        {
            Fox_anim.SetBool("Time_Stop", true);
        }
    }
    ...
    void Die()
    {
        ...
        {
            death_time = 0;
            Fox_anim.SetBool("Die_Cheak", true);
            rb.isKinematic = true;
            Destroy(this.gameObject, 2f);
            G_M.Kill_Count += 1;
            G_M.Kill_Cheak = true;
    ...
    void Enemy_UI()
    {
        HP_Bar.fillAmount = HP / (20*Game_Level); 
