...
public class DragonBoar_Head : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Dragon = DB.transform.GetComponent<DragonBoar>();
    ...
    private void OnTriggerEnter(Collider other)
    {
        if(other.tag == "Arrow" && Play.damage > 0)
        {
            ...
            Dragon.HP -= (Play.damage * Play.Head_M);
            Dragon.Damage_Cheak = true;
            Shot_M.All_False();
            Shot_M.Shot_Text_Wait_Time = Shot_M.Wait_Time_Temp;
            Shot_M.HS = 1;
            Shot_M.Col = true;
            Play.damage = 0;
            Debug.Log("드래곤 보어 머리 피격");

            if((Dragon.HP -= (Play.damage * Play.Head_M)) <= 0)
            {
